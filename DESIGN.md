...
- **Spacing**: viewport gutters leave comfortable inspection space: 32px desktop, 16px mobile; rotated mobile images reserve close-button clearance, use Safari `visualViewport` offsets for height, size dense diagrams from the stable layout width with a `100vw` CSS fallback before height, shrink the frame to the fitted image width, pin the footer control to the visual viewport, and may allow a short vertical scroll under mobile browser chrome; image frame padding is 12px desktop and 8px mobile.

...
TITLE : [토크ON세미나] 추천시스템 분석 입문하기 3강 - 컨텐츠 기반 모델 - Word2Vec(CBOW, Skip-gram) | T아카데미
...
장점과 단점부터 다시 언급하면 장점 같은 경우 직관적인 해석이 가능하다고 했었고 단점 같은 경우 대규모의 말몽치를 가질 때 메모리상의 문제점을 가지고 이게 높은 차원을 가지기 때문에 매트릭스가 되게 스팔스한 형태를 가져서 문제가 생긴다고 이야기 했었습니다.

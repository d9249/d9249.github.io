...
{% include important.html content='해당 함수의 시간복잡도는 1~n 까지 탐색해서 가장 큰 팬케이크를 찾아 제일 위로 올리고 뒤집은 다음 <br>
1~n-1 까지에서 위와 같은 과정을 반복하기 때문에 2 번의 과정이 계속 반복해서 진행되기 때문에 시간복잡도는 O(n)이다. <br>
최악의 경우는(n 번째가 가장 하단) <br>
n 번째에 가장 큰 수를 놓기 위해 2 회 뒤집기 (총 2 회) <br>
n-1 번째에 그 다음 큰 수를 놓기 위해 2 회 뒤집기 (총 4 회) <br>
... <br>

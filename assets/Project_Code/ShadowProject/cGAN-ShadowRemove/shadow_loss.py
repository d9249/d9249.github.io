...
def shadow_region(ori,mask):
    transformer = transforms.Compose([
        transforms.ToTensor(),
        transforms.ToPILImage()])
    
    #a = torch.cuda.FloatTensor(ori)
    print(ori)
    ori = transformer(ori)
...
# results save folder
if not os.path.isdir('loss_shadow_lab'):
    os.mkdir('loss_shadow_lab')
if not os.path.isdir('loss_shadow_lab/Fixed_results'):
    os.mkdir('loss_shadow_lab/Fixed_results')
...
for epoch in range(train_epoch):
    ...
    for x_, y_ in enumerate(data_loader):
        ...
        ###############만들어낸 이미지의 영역을 같은 방법으로 추출해야한다. 
        ###
        
        non_shadow = torch.ones(prediction_fake.size()).to(device)-shadow_mask
        
        loss_g_n = L1_loss(prediction_fake*non_shadow,shadow_free_image*non_shadow)
        loss_g_s = L1_loss(prediction_fake*shadow_mask,shadow_free_image*shadow_mask)
        ...
        #loss_r = 0.795*loss_g+ 0.2*loss_g_s  + 0.0025 * error_real_g + 0.0025 * error_real_p
        loss_r = 0.795*loss_g_n + 0.2*loss_g_s +0.0025 * error_real_g + 0.0025 * error_real_p
        ...
        loss_f = 0.795*loss_g_n + 0.2*loss_g_s +0.0025 * error_fake_g + 0.0025 * error_fake_p
        ...
        if(x_%20 == 0):
            torchvision.utils.save_image(prediction_fake, 'loss_shadow_lab/'+str(epoch)+str(x_)+'test.png')
            #torchvision.utils.save_image(prediction_fake, 'loss_shadow_lab/'+str(epoch)+str(x_)+'test.png')
            torch.save(G.state_dict(), "loss_shadow_lab/"+str(x_)+",generator_param.pkl")
            torch.save(D.state_dict(), "loss_shadow_lab/"+str(x_)+",discriminator_param.pkl")
...
show_train_hist(train_hist, save=True, path='loss_shadow_lab/shadow_train_hist.png')

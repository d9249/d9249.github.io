<template>
  <div>
    <common-header>
      <b-icon icon="file-plus" slot="icon"></b-icon>
      <span slot="title">글쓰기</span>
      <span slot="smallTitle">New post</span>
    </common-header>

    <div class="text pt-1 pb-3">
    ...
    <div id="editor-container" class="text border p-3 mb-3 shadow-sm rounded">
      <editor-tiptap @changeContent="setContent"></editor-tiptap>
    </div>
    ...
    async onClickSubmit() {
      ...
      try {
        ...
        if (this.attachedFiles.length == 0) {
          // 게시판 아이디, 제목, 내용
          const insertResult = await postPost({
        ...
        // 첨부파일이 있으면
          ...
          if (insertResult.data.isOK) {
            tempPostId = insertResult.data.insertId;
            ...
            if (uploadResult.data.isOK) {
              ...
              for (let i = 0; i < this.attachedFiles.length; i++) {
                const result = await postAttachedFile({
                  postId: insertResult.data.insertId,
        ...
        // 게시글 먼저 삽입한 후에, 첨부파일을 관리하기 때문에,
        // 게시글 삽입 후 에러가 발생한다면, 게시글이 남는다. 이를 지워준다.
        if (tempPostId != null)
          await deletePost({ postId: tempPostId });

...
class Baseline():
    def __init__(self, model, num_classes, gpu_id=0, epoch_print=1, print_freq=10, save=False, tag=''):
        self.gpu = gpu_id
            ...
            for i, (X, y) in enumerate(train_data):
                ...
                if (epoch % self.epoch_print == 0) and (i % self.print_freq == 0):
                    ...
                    if self.save and ((self.best_acc == None) or (self.best_acc < test_acc) or (test_loss < self.best_loss)):
                        torch.save(self.model.state_dict(), '{}_{}_{}.pt'.format(self.tag, epoch, i))
...
class Ensemble():
    def __init__(self, Mmodel, Hmodel, LRBmodel, num_classes, gpu_id=0, epoch_print=1, print_freq=10, save=False, tag=''):
        self.gpu = gpu_id
        self.epoch_print = epoch_print
        self.print_freq = print_freq
        self.save = save
        ...
        torch.cuda.set_device(self.gpu)
        self.Mmodel = Mmodel.cuda(self.gpu)
        self.Hmodel = Hmodel.cuda(self.gpu)
        self.LRBmodel = LRBmodel.cuda(self.gpu)
        
        for param in self.Mmodel.parameters(): param.requires_grad = False
        for param in self.Hmodel.parameters(): param.requires_grad = False
        for param in self.LRBmodel.parameters(): param.requires_grad = False
        
        self.classification = nn.Linear(53, num_classes).cuda(self.gpu)
        self.loss_function = nn.CrossEntropyLoss().cuda(self.gpu)
    ...
    def train(self, train_data, test_data, epochs=100, lr=0.1, weight_decay=0.0001):
        optimizer = optim.Adam(self.classification.parameters(), lr, weight_decay=weight_decay)
        
        self.Mmodel.eval(); self.Hmodel.eval(); self.LRBmodel.eval()
        self.classification.train()
        for epoch in range(epochs):
            if epoch % self.epoch_print == 0: print('Epoch {} Started...'.format(epoch+1))
            for i, (X, y) in enumerate(train_data):
                X, y = X.cuda(self.gpu), y.cuda(self.gpu)
                M, H, LRB = self.Mmodel(X), self.Hmodel(X), self.LRBmodel(X)
                mix = torch.cat((M, H, LRB), dim=1)
                output = self.classification(mix)
                loss = self.loss_function(output, y)
                ...
                if (epoch % self.epoch_print == 0) and (i % self.print_freq == 0):
                    ...
                    if self.save and ((self.best_acc == None) or (self.best_acc < test_acc) or (test_loss < self.best_loss)):
                        torch.save(self.classification.state_dict(), '{}_{}_{}.pt'.format(self.tag, epoch, i))
                        self.best_acc = test_acc
                        self.best_loss = test_loss
                        print('Best Model Saved')

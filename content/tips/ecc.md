...
highlights:
  ...
  - "조사 시점 기준 GitHub Releases 최상단은 prerelease `v2.0.0-rc.1`, 최신 stable release는 `v1.10.0`, main의 `package.json`은 `v2.0.0-rc.1`, npm `ecc-universal`은 latest `1.10.0`·next `2.0.0-rc.1`로 표면이 나뉘어 있다."
...
둘째, 버전 표면이 하나로 고정돼 있지 않다. GitHub prerelease `v2.0.0-rc.1`, 최신 stable release `v1.10.0`, main branch `package.json`, npm `latest`/`next`, Claude plugin identifier가 서로 다른 속도로 움직인다. 문서나 예전 글에서 `everything-claude-code`라는 이름을 보더라도 현재 GitHub source repo는 `affaan-m/ECC`, Claude plugin identifier는 `ecc@ecc`, npm package는 `ecc-universal`로 구분하는 편이 안전하다.

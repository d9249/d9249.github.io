...
DeepSpec 저장소 안의 `DSpark_paper.pdf`는 DSpark를 “Confidence-Scheduled Speculative Decoding with Semi-Autoregressive Generation”으로 설명한다. 이름 그대로 두 층이 결합돼 있다.
...
GitHub API와 저장소 내용을 확인한 시점 기준 `deepseek-ai/DeepSpec`은 2026년 6월 26일 만들어진 공개 Python 저장소이며, MIT 라이선스로 배포된다. `LICENSE`, `NOTICE`, `README.md`, `config/`, `deepspec/`, `scripts/`, `train.py`, `eval.py`, `eval_datasets/`, `DSpark_paper.pdf`가 루트에 있다. `config/`는 `dspark`, `dflash`, `eagle3`로 나뉘며, `eval_datasets/`에는 수학·코딩·채팅 benchmark JSONL이 들어 있다. 다만 GitHub tags와 releases는 아직 비어 있었다. 따라서 현재 공개 표면은 안정 릴리스 패키지라기보다 빠르게 공개된 연구·훈련 코드베이스로 읽는 편이 안전하다.
...
Sources: https://discuss.pytorch.kr/t/deepspec-deepseek/11049, https://github.com/deepseek-ai/DeepSpec, https://raw.githubusercontent.com/deepseek-ai/DeepSpec/main/README.md, https://github.com/deepseek-ai/DeepSpec/blob/main/DSpark_paper.pdf, https://huggingface.co/collections/deepseek-ai/deepspec, https://huggingface.co/deepseek-ai/dspark_qwen3_4b_block7, https://huggingface.co/datasets/mlabonne/open-perfectblend

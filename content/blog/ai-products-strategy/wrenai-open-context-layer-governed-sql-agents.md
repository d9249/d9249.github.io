...
LLM에게 데이터베이스를 붙이면 곧바로 BI 에이전트가 되는 것처럼 보일 때가 많다. 하지만 실제 현장에서는 SQL을 _쓸 수 있느냐_ 보다, **그 SQL이 조직의 의미 체계를 제대로 반영하느냐**가 훨씬 더 어렵다. 같은 `revenue`라도 어떤 테이블을 기준으로 보고, 어떤 join 경로를 허용하고, 어떤 계산식을 표준으로 삼고, 어떤 객체는 아예 접근시키지 않을지를 정하지 않으면 모델은 문법적으로 그럴듯한 쿼리를 내놓아도 비즈니스적으로는 틀린 답을 만든다.
...
이 문제 설정은 단순 text-to-SQL보다 한 단계 넓다. 현재 공식 문서의 `what_is_context`는 LLM workflow가 함께 참조해야 할 맥락을 다음처럼 정리한다.

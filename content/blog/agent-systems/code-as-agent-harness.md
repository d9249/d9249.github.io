...
공식 companion repository도 함께 공개되어 있다. 저장소 이름은 **`YennNing/Awesome-Code-as-Agent-Harness-Papers`**이고, README는 이 저장소가 논문을 보조하는 paper list라고 설명한다. 즉 이 저장소는 runnable agent framework라기보다, 논문 taxonomy에 맞춰 대표 연구들을 계속 모으는 curated reading map에 가깝다. README의 news 섹션은 arXiv 공개 소식을 적고, slides와 project page link는 추후 추가하겠다고 안내한다.

확인 가능한 release surface를 정리하면 다음과 같다.
...
| Companion repo | `Awesome-Code-as-Agent-Harness-Papers` | 구현체가 아니라 관련 논문을 taxonomy별로 모으는 resource |
| 라이선스 | GitHub API와 README badge 기준 MIT | companion list의 재사용 조건은 비교적 명확함 |
| 추가 산출물 | README에 slides/project page는 추후 추가 예정이라고 적힘 | 아직 완성된 project package라기보다 초기 공개 상태 |
...
Sources: https://arxiv.org/abs/2605.18747, https://arxiv.org/html/2605.18747v1, https://github.com/YennNing/Awesome-Code-as-Agent-Harness-Papers

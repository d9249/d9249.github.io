...
공개 구현 표면도 어느 정도 확인된다. GitHub API 기준 `1mancompany/OneManCompany` 저장소는 2026-02-26 생성됐고, 조회 시점 기준 2026-05-12까지 push가 이어지고 있었다. stars 222, forks 40, open issues 6이며, GitHub API와 루트 `LICENSE` 모두 Apache-2.0을 가리킨다. latest release는 `v0.7.73`이고, release note는 provider pricing 표시와 Anthropic onboarding 관련 버그 수정을 언급한다.

...
이번 비교를 둘러싼 출발점은 Reddit 커뮤니티에서 빠르게 확산된 요약 글이지만, 핵심 내용의 실체는 Reddit 자체보다 글 작성자가 연결한 `Stet`의 공식 비교 포스트와 methodology 페이지에 있다. 그래서 이 글은 커뮤니티 반응을 중심으로 보기보다, Stet이 공개한 56개 실전 코딩 태스크 비교를 1차 소스로 다시 읽는 쪽에 초점을 맞춘다.

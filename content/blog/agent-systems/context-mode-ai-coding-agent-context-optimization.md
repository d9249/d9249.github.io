...
`mksglu/context-mode`는 바로 이 문제를 "모델이 더 똑똑해지면 해결된다"가 아니라, 런타임이 관리해야 할 인프라 계층이라고 본다. 저장소 설명과 README가 반복해서 강조하는 것도 이 지점이다. 이 프로젝트는 단순한 MCP 툴 하나가 아니라, 샌드박스 실행·세션 연속성·검색 기반 재주입·응답 압축을 묶어서 컨텍스트 창을 운영 자원처럼 아끼는 실행 레이어를 만들려 한다.

흥미로운 점은 범위가 꽤 넓다는 것이다. 공개 README와 설정 문서 기준으로 context-mode는 Claude Code, Gemini CLI, VS Code Copilot, JetBrains Copilot, Cursor, OpenCode, KiloCode 등 다수의 에이전트 하네스를 겨냥한다. 즉 특정 벤더의 기능 확장이 아니라, "AI 코딩 에이전트가 컨텍스트를 다루는 공통 방식"을 만들려는 시도에 가깝다.
...
context-mode가 겨냥하는 문제는 단순히 토큰 절약이 아니다. README의 문제 정의를 보면 네 가지가 함께 묶여 있다. 첫째, MCP 도구 호출이 원시 출력을 그대로 컨텍스트 창에 밀어 넣는 문제다. 둘째, 세션 compact 이후 에이전트가 편집 중인 파일, 최근 사용자 결정, 작업 흐름 같은 상태를 잃는 문제다. 셋째, 모델이 직접 계산과 분류를 수행하려고 하면서 쓸데없이 많은 데이터를 읽는 문제다. 넷째, 장황한 응답 스타일 자체가 또 다른 컨텍스트 낭비가 된다는 문제다.
...
네 번째는 **output compression discipline**이다. `CLAUDE.md`는 "terse like caveman"이라는 거친 슬로건으로 응답 스타일 자체를 압축 규율로 다룬다. 이는 보기엔 단순한 writing rule 같지만, README 문맥에서는 샌드박싱·세션 복구와 함께 컨텍스트 절감의 네 번째 축으로 배치된다. 즉 context-mode는 입력 측면의 낭비만이 아니라 출력 측면의 낭비도 런타임 정책으로 다루고 있다.
...
| Output compression | terse output discipline, auto-expand 예외 규칙 | 응답 자체가 토큰을 과소비하지 않게 제어 |

지원 범위도 꽤 공격적이다. README는 14개 플랫폼을 언급하고, 루트 구조에는 Claude Code용 `.claude-plugin`, OpenClaw용 `openclaw.plugin.json`, PI extension 경로, 여러 adapter별 config와 docs가 들어 있다. `package.json`도 이 프로젝트가 단순 로컬 스크립트가 아니라 CLI, plugin export, OpenClaw extension, skills 디렉터리까지 함께 배포하는 패키지라는 점을 보여준다.

## 공개된 근거에서 확인되는 점

가장 눈에 띄는 근거는 수치다. GitHub API 기준 조회 시점의 저장소는 약 13.2k stars, 901 forks, 13 open issues를 보이고, 최신 릴리스는 `v1.0.111`이다. 루트 페이지는 태그가 이미 140개까지 쌓여 있음을 보여 주며, `stats.json`에는 `users 132.7k+`, `npm 108.9k+`, `marketplace 23.7k+`라는 별도 설치/사용 지표가 들어 있다. star 수와 설치 수는 직접 비교할 수 있는 같은 단위는 아니지만, 적어도 저장소 반응보다 실제 배포 채널 지표를 더 강조하려는 프로젝트라는 점은 분명하다.
...
| 공개 근거 | 확인된 내용 | 해석 |
|---|---|---|
| GitHub API / repo page | 13.2k stars, 901 forks, 140 tags, latest release `v1.0.111` | 빠르게 릴리스되는 툴체인 성격 |
| `stats.json` | `users 132.7k+`, `npm 108.9k+`, `marketplace 23.7k+` | GitHub 지표 외에 설치/유통 지표를 별도로 운영 |
...
이 해석은 멀티 하네스 지원과도 연결된다. Claude Code, Cursor, Gemini CLI, Copilot, OpenCode처럼 각 환경의 hook 체계와 MCP 연결 방식은 다르지만, 컨텍스트 낭비라는 문제 자체는 공통이다. context-mode는 이 공통 병목을 겨냥해 "라우팅 규칙 + 로컬 지식 저장소 + 샌드박스 계산 + terse output"이라는 공통 해법을 이식하려 한다. 즉 특정 모델 최적화라기보다, 에이전트 런타임 운영 원칙을 패키징한 프로젝트에 가깝다.

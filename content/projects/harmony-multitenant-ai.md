...
<figure class="harmony-diagram">
  ...
  <figcaption>요청은 UserContext와 Session으로 정규화되고, 에이전트는 persona, tool context, citation, audit을 하나의 turn 결과로 남깁니다.</figcaption>
</figure>
...
<figure class="harmony-diagram">
  ...
  <figcaption>개인 채팅, Space 채팅, Voice meeting은 같은 에이전트로 들어가지만 검색 후보 합성은 session channel과 Space policy에 따라 달라집니다.</figcaption>
</figure>
...
<figure class="harmony-diagram">
  ...
  <figcaption>문서와 회의 산출물은 compiler, review queue, approval/publish, index reload를 거쳐야 공식 근거가 됩니다.</figcaption>
</figure>
...
<figure class="harmony-diagram">
  ...
  <figcaption>AWS deployment topology. edge/auth 경계 뒤 private VPC의 Fargate 에이전트가 Aurora, EFS PageIndex, S3, Bedrock Runtime을 사용합니다.</figcaption>
</figure>

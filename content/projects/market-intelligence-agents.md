...
runtime의 중심에는 workflow orchestrator가 있습니다. 입력으로 특정 RFP 문서가 오면 해당 문서를 파싱하고, 없으면 `00_input_data/`에서 최신 RFP 문서를 찾습니다. scheduled run에서는 `00_input_data/`의 모든 문서를 순회하며 문서별 workflow를 실행합니다.
...
공통 agent runtime은 KB Intelligence agent들의 실행 계약입니다. 각 agent는 `collect`, `analyze`, `evaluate` 단계를 따릅니다. collect는 Brave Search나 RFP parser처럼 외부 데이터를 가져오는 단계이고, analyze는 Bedrock 모델을 호출해 agent별 JSON schema를 채우는 단계입니다. evaluate는 결과 품질을 점수화하고, 부족하면 개선 피드백을 만들어 다음 분석 prompt에 다시 넣습니다.
...
<div class="article-table-wrap market-table-wrap">
  <table>
    ...
    <tbody>
      ...
      <tr>
        <td>KB 동기화</td>
        <td>Knowledge Base sync, Bedrock Knowledge Base ingestion</td>
